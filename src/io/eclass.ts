/**
 * e-class (LMS) のアンケート教材が書き出す希望順位ファイルのパーサ。
 *
 * ファイルの形:
 *
 * ```
 * 5〜6 行   ヘッダ（教材名、出力日時など）— 使わない。行数は出力オプション次第
 * （空行）
 * 2 行      問題のパラメータ。後半の列に選択肢ラベル option1 … option26 が入る
 *           ラベルは <p tagname="p">○○研究室（○○　○○）</p> のような HTML
 * （空行）
 *           [回答一覧]                            …ブロック
 *           [アンケート集計]                      …ブロック
 *           [ユーザ毎の回答リスト]                …ブロック（ここから希望順位を取る）
 ...
 *
 * 行数は決め打ちにせず、空行と `[...]` の見出しで区切って数える。e-class の版が
