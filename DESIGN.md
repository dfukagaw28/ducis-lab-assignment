...
任意で、無ければ「学生数 ÷ 研究室数」の切り上げを全研究室に入れ、そう決めたことを
警告に出す（`io/intake.ts` の `resolveCapacities`）。定員は運用上の決定なので、黙って
決めてしまわないのが要点。一部の研究室だけ空欄なのは書き忘れとみて止める。
...
| 定員 | 研究室 CSV、無ければ学生数 ÷ 研究室数 の切り上げ | 画面から上書きできるようにしたい |
