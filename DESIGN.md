...
シードそのものは、既定では**入力データから導きます**（`domain/seed.ts`）。人が選べると
結果を見てから選び直した疑いが残るためで、選ぶ人がいなければ疑う余地もありません。
定員の自動配分がシードを使うので、ハッシュは**定員を決める前の入力**から取ります
（そうしないと循環します）。手で指定した場合は、そのことがサマリに残ります。
...
- e-class の回答 `4, 1, 3, 2` が「option1 が 4 位」であって「1 位が option4」ではない
  （どちらに読んでも例外は出ず、希望順位だけが丸ごと入れ替わる）
