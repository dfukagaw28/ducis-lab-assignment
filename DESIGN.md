...
行数は決め打ちにせず、空行と `[...]` の見出しで区切って数える。上の 5 行と 2 行は
目安で、違っていても通し、`sections.notes` に書き出すだけにする（`npm run inspect`
が「注意」として表示する）。eClass の版が変わって前置きの行数が動いても止まらない。
